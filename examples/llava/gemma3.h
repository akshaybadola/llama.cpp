...
struct gemma3_context {
    ...
    void init_vision_context(common_params & params) {
        ...
        ctx_vision.reset(mtmd_init_from_file(clip_path, model, mtmd_context_params_default()));
        if (!ctx_vision.get()) {
    ...
    /* mtmd_bitmap * load_image_from_buf(const unsigned char * buf, size_t len) { */
...
static common_params init_default_params() {
    ...
    if (!common_params_parse(1, argv, params, LLAMA_EXAMPLE_SERVER)) {
        return params;
    }
...
static int eval_message_with_images(gemma3_context & ctx,
    ...
    for (auto & image_buf : images) {
        bitmaps.entries.push_back(mtmd_helper_bitmap_init_from_buf(image_buf.data, image_buf.size));
    }
    ...
    int32_t res = mtmd_tokenize(ctx.ctx_vision.get(), chunks.ptr.get(),
                                &text,
                                bitmaps_c_ptr.data(),
                                bitmaps_c_ptr.size());
    if (res != 0) {
    ...
    if (mtmd_helper_eval_chunks(ctx.ctx_vision.get(), ctx.lctx, chunks.ptr.get(),
                                ctx.n_past, 0, ctx.n_batch, true, &new_n_past)) {
